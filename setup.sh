...
install() {
  ...
  $APM_PKG_INSTALL_DIR/python/bin/pip3.9 install frida==16.0.2 frida-tools==12.0.1
  ln -s $APM_PKG_INSTALL_DIR/python/bin/frida* $APM_PKG_BIN_DIR/
}

uninstall() {
  rm -rf $APM_PKG_BIN_DIR/python
  rm $APM_PKG_BIN_DIR/frida*
}
